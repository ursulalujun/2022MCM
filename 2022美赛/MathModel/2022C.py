...
# 9.28开始
df=pd.read_csv('LBMA-GOLD.csv',na_values='NAN').iloc[12:,1:]
df=scaler(df)
df2=pd.read_csv('gold.csv').iloc[:,1:]
df2=scaler(df2)
...
# economic_feature()
X,y=sliding_window(df['USD (PM)'].tolist(), len_train, len_pre, df2)
X=np.array(X)
y=np.array(y)
...
y1_plot=df['USD (PM)'].iloc[len_train:1012]
y2_plot=df['USD (PM)'].iloc[1012+len_train:]
